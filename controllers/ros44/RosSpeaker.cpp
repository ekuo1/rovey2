...
RosSpeaker::RosSpeaker(Speaker *speaker, Ros *ros) : RosDevice(speaker, ros) {
  ...
  mGetEngineServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_engine", &RosSpeaker::getEngineCallback);
  mGetLanguageServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_language", &RosSpeaker::getLanguageCallback);
  mSetEngineServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/set_engine", &RosSpeaker::setEngineCallback);
  mSetLanguageServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/set_language", &RosSpeaker::setLanguageCallback);
  mSpeakServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/speak", &RosSpeaker::speakCallback);
  mPlayServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/play_sound", &RosSpeaker::playCallback);
  mSpeaker = speaker;
}
