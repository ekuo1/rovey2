...
RosReceiver::RosReceiver(Receiver *receiver, Ros *ros) : RosSensor(receiver->getName(), receiver, ros) {
  ...
  mSetChannelServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/set_channel", &RosReceiver::setChannelCallback);
  mGetChannelServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_channel", &RosReceiver::getChannelCallback);
  mGetQueueLengthServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_queue_length", &RosReceiver::getQueueLengthCallback);
  mNextPacketServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/next_packet", &RosReceiver::nextPacketCallback);
  mGetDataSizeServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_data_size", &RosReceiver::getDataSizeCallback);
  mGetSignalStrengthServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_signal_strength", &RosReceiver::getSignalStrengthCallback);
  mGetEmitterDirectionServer = RosDevice::rosAdvertiseService((ros->name())+'/'+fixedDeviceName+"/get_emitter_direction", &RosReceiver::getEmitterDirectionCallback);
}
...
bool RosReceiver::getEmitterDirectionCallback(webots_ros::receiver_get_emitter_direction::Request &req, webots_ros::receiver_get_emitter_direction::Response &res) {
  if (mReceiver->getQueueLength() > 0)
  {
