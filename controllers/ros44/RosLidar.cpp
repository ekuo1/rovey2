...
RosLidar::RosLidar(Lidar *lidar, Ros *ros) : RosSensor(lidar->getName(), lidar, ros) {
  ...
  mEnablePointCloudServer = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"enable_point_cloud", &RosLidar::enablePointCloudCallback);
  mGetFrequencyInfoServer = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"get_frequency_info", &RosLidar::getFrequencyInfoCallback);
  mGetInfoServer = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"get_info", &RosLidar::getInfoCallback);
  mIsPointCloudEnabledServer = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"is_point_cloud_enabled", &RosLidar::isPointCloudEnabledCallback);
  mSetFrequencyServer = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"set_frequency", &RosLidar::setFrequencyCallback);
  mGetLayerRangeImage = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"get_layer_range_image", &RosLidar::getLayerRangeImage);
  mGetLayerPointCloud = RosDevice::rosAdvertiseService((ros->name())+'/'+deviceNameFixed+'/'+"get_layer_point_cloud", &RosLidar::getLayerPointCloud);
}
...
ros::Publisher RosLidar::createPublisher() {
  ...
  for (int i = 0; i < mLidar->getNumberOfLayers(); ++i) {
    std::ostringstream s;
    s << i;
    mLaserScanPublisher[i] = RosDevice::rosAdvertiseTopic(mRos->name()+'/'+deviceNameFixed+"/laser_scan/layer"+s.str(), LaserScaneType);
  }
...
void RosLidar::publishValue(ros::Publisher publisher) {
  ...
  rangeImageVector = (const char *)mLidar->getRangeImage();
  ...
  for (unsigned int i = 0; i < sizeof(float) * mLidar->getHorizontalResolution() * mLidar->getNumberOfLayers(); ++i)
...
bool RosLidar::isPointCloudEnabledCallback(webots_ros::node_get_status::Request &req, webots_ros::node_get_status::Response &res) {
  res.status = mLidar->isPointCloudEnabled();
  return true;
}

bool RosLidar::getFrequencyInfoCallback(webots_ros::lidar_get_frequency_info::Request &req, webots_ros::lidar_get_frequency_info::Response &res) {
...
bool RosLidar::getLayerRangeImage(webots_ros::lidar_get_layer_range_image::Request &req, webots_ros::lidar_get_layer_range_image::Response &res) {
  const char *rangeImageVector = (const char *)mLidar->getLayerRangeImage(req.layer);
  ...
  for (unsigned int i = 0; i < sizeof(float) * mLidar->getHorizontalResolution(); ++i)
    res.image.data.push_back(rangeImageVector[i]);
  return true;
}

bool RosLidar::getLayerPointCloud(webots_ros::lidar_get_layer_point_cloud::Request &req, webots_ros::lidar_get_layer_point_cloud::Response &res) {
  if (!mIsPointCloudEnabled)
    return false;
