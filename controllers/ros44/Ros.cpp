...
void Ros::launchRos(int argc,char **argv) {
  ...
  mWaitForUserInputEventService = mNodeHandle->advertiseService(mRobotName+"/robot/wait_for_user_input_event", &Ros::waitForUserInputEventCallback, this);
  ...
  mGetCustomDataService = mNodeHandle->advertiseService(mRobotName+"/robot/get_custom_data", &Ros::getCustomDataCallback, this);
  mSetCustomDataService = mNodeHandle->advertiseService(mRobotName+"/robot/set_custom_data", &Ros::setCustomDataCallback, this);
  ...
  mWwiReceiveTextService = mNodeHandle->advertiseService(mRobotName+"/robot/wwi_receive_text", &Ros::wwiReceiveTextCallback, this);
...
void Ros::setupRobot() {
  wb_robot_init();
  int type = wb_robot_get_type();
  if (type == Node::DIFFERENTIAL_WHEELS)
    mRobot = new DifferentialWheels();
  else if (type == Node::SUPERVISOR)
    mRobot = new Supervisor();
  else
    mRobot = new Robot();
}

Device * Ros::getDevice(std::string name) {
  for (unsigned int i = 0; i < mDeviceList.size(); ++i) {
...
bool Ros::getDeviceListCallback(webots_ros::robot_get_device_list::Request &req, webots_ros::robot_get_device_list::Response &res) {
  int nDevices = mRobot->getNumberOfDevices();
...
bool Ros::waitForUserInputEventCallback(webots_ros::robot_wait_for_user_input_event::Request &req, webots_ros::robot_wait_for_user_input_event::Response &res) {
